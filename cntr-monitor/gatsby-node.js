...
exports.createPages = async function ({ actions, graphql }) {
  ...
  issues.forEach((node) => {
    ...
    const postTemplate = require.resolve(`./src/components/Issue.js`)
    const translations = findTranslationNodes(node, issues)
  ...
  posts.forEach((post) => {
    ...
    const translationIds = translations.map((t) => t.id)
    const year = post.relativeDirectory.replace(/(.{2})\/(issues)\//g, '').replace('/posts', '')
    let path = `${locale && locale !== defaultLocale ? locale : ''}/${year}/${post.childMdx.fields.slug}`

    const issue = issues.find((node) => {
      return node.relativeDirectory === post.relativeDirectory.replace('/posts', '')
    })

    actions.createPage({
      path: path,
      component: `${postTemplate}?__contentFilePath=${post.childMdx.internal.contentFilePath}`,
      context: { id: post.id, translations: translationIds, issueId: issue.id },
    })
  })
