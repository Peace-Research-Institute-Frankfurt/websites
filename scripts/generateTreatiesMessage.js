...
try {
  differences.forEach((d) => {
    ...
    if (ti === -1) {
      friendlyDiff.treaties.push({ name: headTreaty.name, title: headTreaty.title, countries: [{ cca2: headCountry.cca2, title: headCountryData.name.common, countryMessage: countryMessage, changes: eventMessages }] })
    } else {
      const ci = friendlyDiff.treaties[ti].countries.findIndex((el) => el.cca2 === headCountry.cca2)
      if (ci === -1) {
        friendlyDiff.treaties[ti].countries.push({ cca2: headCountry.cca2, title: headCountryData?.name?.common, countryMessage: countryMessage, changes: eventMessages })
      } else {
